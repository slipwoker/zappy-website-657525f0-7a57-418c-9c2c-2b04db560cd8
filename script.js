document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        error = getErrorMessage('required', name);
      } else if (field.type === 'email' && value !== '') {
        if (!isValidEmail(value)) {
          error = getErrorMessage('email', name);
        }
      } else if (field.type === 'tel' && value !== '') {
        if (!isValidPhone(value)) {
          error = getErrorMessage('phone', name);
        }
      } else if (field.hasAttribute('minlength')) {
        const min = parseInt(field.getAttribute('minlength'), 10);
        if (value.length < min) {
          error = getErrorMessage('minlength', name, min);
        }
      } else if (field.hasAttribute('maxlength')) {
        const max = parseInt(field.getAttribute('maxlength'), 10);
        if (value.length > max) {
          error = getErrorMessage('maxlength', name, max);
        }
      }
  ...
  document.body.addEventListener('blur', function (e) {
    ...
    if (field.hasAttribute('required') && value === '') {
      error = getErrorMessage('required', field.name);
    } else if (field.type === 'email' && value !== '') {
      if (!isValidEmail(value)) {
        error = getErrorMessage('email', field.name);
      }
    } else if (field.type === 'tel' && value !== '') {
      if (!isValidPhone(value)) {
        error = getErrorMessage('phone', field.name);
      }
    }
    ...
    const mutationObserver = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          ...
          const animatableNodes = node.matches && node.matches(OBSERVED_SELECTOR)
            ? [node]
            : Array.from(node.querySelectorAll ? node.querySelectorAll(OBSERVED_SELECTOR) : []);

          if (animatableNodes.length) {
            initScrollAnimations();
          }
        });
      });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אברהם בלום משרד עורכי דין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '657525f0-7a57-418c-9c2c-2b04db560cd8',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '657525f0-7a57-418c-9c2c-2b04db560cd8';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '657525f0-7a57-418c-9c2c-2b04db560cd8';
